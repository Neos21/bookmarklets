/** ../public/src/・../public/dist/ ディレクトリ配下のファイルを利用し、./scripts/build-index.html をベースに HTML を構築して ../pubic/index.html を生成する */
...
const contents = fs.readdirSync(path.resolve(__dirname, '../public/src/')).map((fileName) => {
  const id = fileName.replace((/\.js$/u), '');  // 拡張子を除去して ID に使用する
  
  const srcFile = fs.readFileSync(path.join(__dirname, '../public/src/', fileName), 'utf-8');  // JS ファイルを読み込む
  ...
  if(!isDisablePrintDistScript) {
    const distFile = fs.readFileSync(path.join(__dirname, '../public/dist/', fileName), 'utf-8');
...
// index.html を出力する
fs.writeFileSync(path.resolve(__dirname, '../public/index.html'), distIndexHtml, { encoding: 'utf-8' });
